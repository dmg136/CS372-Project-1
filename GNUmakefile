...
TARGETS := hi hello words fact execUs testPoint testSortedPoints
...
handin.tar: clean
	tar cf handin.tar `find . -type f | grep -v '^\.*$$' | grep -v '/CVS/' | grep -v '/\.svn/' | grep -v '/\.git/' | grep -v 'proj[0-9].*\.tar\.gz' | grep -v '/\~/' | grep -v 'C.html' | grep -v '/\.tar/'` 
...
execUs.o: execUs.c
	$(CC) $(CFLAGS) -c -o execUs.o execUs.c
